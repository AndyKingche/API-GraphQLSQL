...
module.exports = {
    Comentarios :{
        usuariosid: async({ usuariosid }) => {
            ...
            usuario = id > 0 ? await  db.sequelize3.query("SELECT * FROM usuarios Where id=:id",{
        ...
        postsid:async({postsid}) =>{
            ...
            post = id > 0 ? await  db.sequelize3.query("SELECT * FROM posts Where id=:id",{
                    mapToModel: true, // pass true here if you have any mapped fields
                    nest: true,
                    replacements: { id:id }
                  }) : Array(post)              
    ...
    Posts:{
        categoriasid: async ( {categoriasid} ) => {
            ...
            try {
                ...
                categoria = id > 0 ? await  db.sequelize3.query("SELECT * FROM categorias Where id=:id",{
                    mapToModel: true, // pass true here if you have any mapped fields
                    nest: true,
                    replacements: { id:id }
                  }): Array(categoria)
        ...
        comentarios: async ( {id} ) =>{
            ...
            let comentario
            try {
                    comentario = await  db.sequelize3.query("SELECT * FROM comentarios Where postsid=:id",{
                    mapToModel: true, // pass true here if you have any mapped fields
                    nest: true,
                    replacements: { id:id }
                  })
